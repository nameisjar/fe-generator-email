...
async function confirmRemove() {
  ...
  deleting.value = true;
  try {
    await emailStore.remove(emailToDelete.value.id);
    emailToDelete.value = null;
    toast.show('Email berhasil dihapus.');
  } catch (error) {
    toast.show(error.response?.data?.error?.message || 'Email gagal dihapus.', 'error');
  } finally {
...
<template>
  <div class="space-y-4">
      ...
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2 text-xs text-slate-500 mb-1">
            <RouterLink :to="{ name: 'aliases' }" class="hover:text-brand-600">Alamat alias</RouterLink>
            <span>/</span>
            <span class="text-slate-700">{{ alias?.label || alias?.address || 'Kotak masuk' }}</span>
          </div>
          <h1 class="text-2xl font-bold tracking-tight text-slate-900 truncate">
            {{ alias?.label || alias?.address || 'Kotak masuk' }}
          </h1>
          <button
            v-if="alias"
          ...
          >
            ...
            <span v-else class="text-emerald-600 text-xs font-sans">✓ Disalin</span>
          </button>
          <p v-else class="text-sm text-slate-500">Memuat alias…</p>
        </div>
    ...
    <div v-if="emailStore.loading && !list.items.length" class="card p-12 text-center">
      ...
      <p class="text-sm text-slate-500 mt-3">Memuat email…</p>
    </div>
    ...
    <div v-else-if="!list.items.length" class="card p-12 text-center">
      ...
      <p class="text-slate-700 font-medium mt-3">
        {{ search ? 'Tidak ada hasil' : showUnreadOnly ? 'Semua sudah dibaca' : 'Kotak masuk masih kosong' }}
      </p>
      <p class="text-sm text-slate-500 mt-1">
        {{ search
          ? 'Coba kata pencarian lain atau hapus filter.'
          : showUnreadOnly
            ? 'Tidak ada pesan yang belum dibaca untuk alias ini.'
            : 'Kirim email ke alamat alias ini untuk melihatnya di sini.' }}
      </p>
      <button
    ...
    <ConfirmDialog
      ...
      :busy="deleting"
      title="Hapus email?"
      description="Email yang dihapus tidak dapat dipulihkan."
